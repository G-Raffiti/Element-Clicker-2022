...
{
    public abstract class DictionaryDrawer<TK, TV> : UnityEditor.PropertyDrawer
    {
        private GenericDictionary<TK, TV> _Dictionary;
        ...
        public override void OnGUI(Rect position, UnityEditor.SerializedProperty property, GUIContent label)
        {
            ...
            _Foldout = UnityEditor.EditorGUI.Foldout(foldoutRect, _Foldout, label, true);
            if (UnityEditor.EditorGUI.EndChangeCheck())
                UnityEditor.EditorPrefs.SetBool(label.text, _Foldout);

            var buttonRect = position;
            buttonRect.x = position.width - kButtonWidth + position.x;
            buttonRect.width = kButtonWidth + 2;

            if (GUI.Button(buttonRect, new GUIContent("+", "Add item"), UnityEditor.EditorStyles.miniButton))
            {
                AddNewItem();
            }
            ...
            if (GUI.Button(buttonRect, new GUIContent("X", "Clear dictionary"), UnityEditor.EditorStyles.miniButtonRight))
            {
                ClearDictionary();
            }
            ...
            foreach (var item in _Dictionary)
            {
                ...
                if (GUI.Button(removeRect, new GUIContent("x", "Remove item"), UnityEditor.EditorStyles.miniButtonRight))
                {
                    RemoveItem(key);
                    break;
                }
        ...
        private void CheckInitialize(UnityEditor.SerializedProperty property, GUIContent label)
        {
            if (_Dictionary == null)
            {
                var target = property.serializedObject.targetObject;
                _Dictionary = fieldInfo.GetValue(target) as GenericDictionary<TK, TV>;
                if (_Dictionary == null)
                {
                    _Dictionary = new GenericDictionary<TK, TV>();
                    fieldInfo.SetValue(target, _Dictionary);
                }

                _Foldout = UnityEditor.EditorPrefs.GetBool(label.text);
            }
        }
        ...
        private void AddNewItem()
        {
            ...
            if (typeof(TK) == typeof(string))
                key = (TK)(object)"";
            else if (typeof(TK) == typeof(EResource))
            {
                foreach (TK dictionaryKey in Enum.GetValues(typeof(EResource)))
                {
